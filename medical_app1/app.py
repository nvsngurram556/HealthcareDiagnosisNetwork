...
import io
import json
import requests # Added for making API calls to Gemini
from flask_cors import CORS # Added for handling CORS issues with frontend

# Initialize Flask app
app = flask.Flask(__name__)
# Enable CORS for all routes, crucial for frontend-backend communication in development environments
CORS(app)
...
def add_relationship_to_graph(source_entity, relationship_type, target_entity, source_type=None, target_type=None):
    ...
    if source_entity not in G:
        ...
        # Update type if a more specific type is provided or if it's currently unknown
...
def query_graph():
    ...
    # Get the entity type from the graph if it exists
    ...
    if query_type == "diagnoses_for_symptom":
        ...
        for neighbor in G.successors(entity):
            if G[entity][neighbor].get('relationship') == "indicates" and G.nodes[neighbor].get('type') == 'disease':
        ...
        for neighbor in G.successors(entity):
            if G[entity][neighbor].get('relationship') == "treated_by" and G.nodes[neighbor].get('type') == 'treatment':
                results.append(neighbor)
    ...
    return jsonify({"status": "success", "results": results, "message": message, "entity_type": entity_type})


@app.route('/explain_entity', methods=['POST'])
def explain_entity():
    """
    API endpoint to get an LLM explanation for a given entity.
    Expected JSON payload:
    {
        "entity": "Fever"
    }
    ...
    if not entity:
        return jsonify({"status": "error", "message": "Entity for explanation is missing."}), 400

    if entity not in G:
        return jsonify({"status": "info", "message": f"'{entity}' not found in the knowledge graph. Cannot provide LLM explanation."}), 200

    entity_type = G.nodes[entity].get('type', 'medical concept') # Default to 'medical concept' if type not explicitly set

    prompt = f"Explain '{entity}' in a concise way, considering it is a {entity_type} in a medical context. Keep the explanation brief, around 2-3 sentences. Do not mention its relationships within a graph."

    try:
        chat_history = []
        chat_history.append({ "role": "user", "parts": [{ "text": prompt }] })
        payload = { "contents": chat_history }
        api_key = "" # Canvas will provide this automatically. DO NOT hardcode your API key here.
        ...
        if result.get('candidates') and len(result['candidates']) > 0 and \
           ...
           len(result['candidates'][0]['content']['parts']) > 0:
            ...
            return jsonify({"status": "success", "explanation": llm_text})
        else:
            return jsonify({"status": "error", "message": "LLM response was empty or malformed."}), 500

    except requests.exceptions.RequestException as e:
        print(f"Error calling Gemini API: {e}")
        return jsonify({"status": "error", "message": f"Failed to get explanation from LLM. API error: {str(e)}"}), 500
    except Exception as e:
        print(f"An unexpected error occurred during LLM explanation: {e}")
        return jsonify({"status": "error", "message": f"An unexpected error occurred: {str(e)}"}), 500

# To run the Flask app:
# 1. Save this code as `app.py`.
# 2. Open your terminal, navigate to the directory where you saved `app.py`.
# 3. Run `pip install Flask networkx pandas requests flask_cors` if you haven't already.
